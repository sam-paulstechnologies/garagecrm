...
class SettingsController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin.settings.index', [
            'company' => $company,
            'settings' => collect($store->all())->except([
                'meta.access_token', 'meta.app_id', 'meta.page_id', 'meta.form_id', 'meta.form_ids',
                'twilio.account_sid', 'twilio.auth_token', 'twilio.whatsapp_from',
            ])->all(),
    ...
    {
        $company = $this->company();
        $hasConnection = MetaPage::query()->where('company_id', $company->id)->exists();

        if (! $hasConnection) {
            return back()->with(
                'warning',
                'Meta is not connected. Use the secure Meta connection flow.'
            );
        }
    ...
    public function testTwilioInline()
    {
        ...
        $hasSid = filled(config('services.twilio.sid'));
        $hasToken = filled(config('services.twilio.auth_token'));
        $hasFrom = filled(config('services.twilio.whatsapp_from'));

        if (! $hasSid || ! $hasToken || ! $hasFrom) {
            return back()->with(
                'warning',
                'Twilio is not configured by the platform operator.'
            );
        }
