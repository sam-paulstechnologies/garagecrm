...
Azure staging is provisioned in `rg-sayaraforce-staging`; `app-sayaraforce-staging` serves `https://staging.sayaraforce.com` independently from production. Provisioning and deployment scripts still perform independent identity checks and stop before any write when the subscription, resource group, exact staging app, or region is ambiguous.
...
The pending migrations run in reviewed order after the baseline. `2026_08_14_000002_create_whatsapp_history_intelligence` adds tenant-scoped history quarantine/review, cumulative contact usage, privacy preferences, safety audits, and generic commercial usage events; the following mode-only reconciliation migration fail-closes a dotted capability-key lookup defect without changing prices, subscriptions, invoices or provider mappings. `2026_08_14_000004_create_quick_scan_workspaces` adds six temporary, platform-created scan/quarantine/audit tables without creating tenants, subscriptions or customer CRM rows. The five 2026-08-15 security migrations add private-storage provenance, complete missing clean-install migration paths, and a keyed lookup hash for encrypted webhook verification tokens. Two independent fresh cycles now produce 141 base tables, two valid views, 59 migration records, two explicitly subscribed synthetic tenants, zero foreign-key violations across 202 constraints, 498 routes, and identical fingerprint `d086c533e6898a6a8bb5ac782585aeb7e7693da2cba5ebda2587c8d791c71b30`.
...
Quick Scan is explicitly staging-enabled with a 500-contact internal analysis ceiling, 24-hour initiation link, 72-hour report window, 24-hour declined-data cooling period and versioned consent policy. Application/default configuration remains disabled. These are acquisition safety controls, not tenant-plan entitlements or public commercial promises.
...
Public garage self-registration is controlled only by `config('registration.public_enabled')`, backed by `PUBLIC_REGISTRATION_ENABLED`. The default is `false`; the staging Bicep template explicitly sets it to `true`. A successful submission atomically creates one Company, one Garage, and one administrator, signs in that administrator, and redirects to the WhatsApp onboarding screen. It does not create a messaging connection, provider record, consent, onboarding session, or outbound message.

Staging sets `TWO_FACTOR_ENFORCEMENT=required_admins`. Platform administrators and tenant administrators without confirmed two-factor authentication are confined to the enrollment flow; normal operational roles may enroll voluntarily. Sensitive billing, WhatsApp connection, user/role, platform, and security mutations require a configurable recent password-and-TOTP step-up (15 minutes in staging). Production rollout remains separately controlled through `off` to `audit` to `required_admins`; the staging value is not a global default.
...
The workflow creates an ephemeral test-only application key without printing it, validates the static baseline/cutoff, lints PHP, runs focused staging/WhatsApp tests, runs the complete suite, builds frontend assets, removes the temporary `.env`, installs optimized production Composer dependencies, creates a Linux-portable ZIP with forward-slash entry names, verifies the exact staging Azure resource ID, records branch/commit/time, and deploys only to the staging app. A guarded triggered WebJob then creates Laravel's writable runtime directories, runs pending staging migrations, idempotently seeds only the canonical commercial catalogue, assigns an explicit subscription only to still-unassigned synthetic staging companies, verifies the approved fingerprint, and rebuilds caches inside the PHP container. Routine application deployments never rerun the operational synthetic seeder. Only after post-deployment validation succeeds is the queue worker installed. The scheduler remains absent.
