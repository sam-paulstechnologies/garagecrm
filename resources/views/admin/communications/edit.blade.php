...
<div class="max-w-3xl mx-auto p-6">
  ...
  <form method="post" action="{{ route('admin.communications.update', $communication) }}" class="bg-white p-6 rounded shadow space-y-4">
    @csrf @method('PUT')

    <div>
      <label class="block text-sm font-medium mb-1">Client</label>
      <select class="border rounded p-2 w-full" disabled>
        <option>{{ optional($communication->client)->name ?? '—' }}</option>
      </select>
    </div>
    ...
    <div class="pt-2">
      ...
      <a href="{{ route('admin.communications.show', $communication) }}" class="px-4 py-2 bg-gray-100 rounded">Cancel</a>
    </div>
  </form>
</div>
@endsection
