...
- Together they produce 128 base tables in the current validated fresh database.
...
The guarded validator refuses a non-local host, a database name without `staging_validation`, or the normal development database. Each cycle dropped and recreated only the named disposable validation database, loaded the canonical baseline through Laravel, ran all six reviewed post-baseline migrations, seeded the immutable five-plan catalogue and two synthetic tenants, assigned explicit synthetic subscriptions, verified 159 foreign-key constraints, created and queried both views, booted Laravel, discovered 459 routes, and ran the representative application-surface integration test.
...
| Foreign-key constraints checked | 165 | 165 |
| Foreign-key violations | 0 | 0 |
| Structural fingerprint | `30898ba3...91ce8` | `30898ba3...91ce8` |
