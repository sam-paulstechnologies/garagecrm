...
| A.5.3 Segregation of duties | Yes | Partial | tenant/platform separation, step-up; formal release approver evidence required |
...
| A.5.19–A.5.23 Supplier/cloud security | Yes | Partial | supplier register and policy; contracts/DPA review open |
...
| A.8.3 Information access restriction | Yes | Implemented | tenant scopes, policies, entitlements, private files |
| A.8.4 Source-code access | Yes | Partial | repository controls assumed; branch-protection/admin inventory evidence required |
| A.8.5 Secure authentication | Yes | Implemented | Fortify/TOTP/throttling/session controls |
...
| A.8.13 Information backup | Yes | Partial | Azure backup configured; restore evidence open |
...
| A.8.24 Cryptography | Yes | Implemented/Partial | TLS/KV/encrypted fields; key lifecycle evidence needs recurring review |
| A.8.25–A.8.31 Secure SDLC/coding/testing/separation | Yes | Implemented | clean worktree, staging-only release, tests, scans, environment isolation |
