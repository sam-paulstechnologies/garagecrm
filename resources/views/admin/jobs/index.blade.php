  ...
  <form method="GET" action="{{ route('admin.jobs.index') }}" class="grid md:grid-cols-3 gap-3 mb-4">
    <div class="md:col-span-2 flex">
      <input type="text"
             name="q"
             value="{{ $q ?? '' }}"
             placeholder="Search job code, description, client…"
             class="w-full border rounded-l px-3 py-2" />

      <button class="border border-l-0 rounded-r px-4 py-2 bg-gray-50">
        Search
      </button>
    </div>
    ...
    <div class="flex items-center gap-2">
      ...
      <a href="{{ route('admin.jobs.index') }}"
         class="text-sm text-gray-600 underline whitespace-nowrap">
        Reset
      </a>
        ...
        <tr class="border-t hover:bg-gray-50">
          ...
          <td class="px-4 py-2">
            <div class="flex justify-end gap-3">
              ...
              <form action="{{ route('admin.jobs.archive', $job->id) }}"
                    method="POST"
                    class="inline-block"
