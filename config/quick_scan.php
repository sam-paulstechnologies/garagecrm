...
return [
    ...
    // Fable M2: canonical retention window for customer-level scan data. The
    // reconciliation sweep derives each scan's purge deadline as its retention
    // anchor (history_sync_started_at, else created_at) plus this window, so any
    // non-converted scan holding customer data is physically purged once the
    // window elapses, even if it stalled/failed before report_ready.
